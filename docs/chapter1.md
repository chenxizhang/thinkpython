...
> 🎇陈希章：**解决问题** 这个能力是最重要的，不管你是不是从事计算机和软件开发工作。**计算思维** 是另外一个术语，这是由原卡耐基·梅隆大学（CMU）计算机系主任周以真教授（Jeannette M. Wing）在2006年首次提出，并发表在《Communications of the ACM》杂志。周教授认为：计算思维是运用计算机科学的基础概念进行问题求解、系统设计、以及人类行为理解等涵盖计算机科学之广度的一系列思维活动。有关计算思维的更多说明，请参考 [这里](https://baike.baidu.com/item/%E8%AE%A1%E7%AE%97%E6%80%9D%E7%BB%B4/4689456) 的介绍。
...
* 条件运算：检查特定条件是否满足来运行相应的代码。

* 重复操作：重复进行一些操作，通常每次操作会有些变化。
...
>🎇陈希章：请再次想一想前面提到的 **计算思维** 和 **解决问题** 的能力。
...
>🎇陈希章：由于一些网络层面的原因，`pythonanywher.com` 这个网站在国内访问速度比较慢，所以我还是建议本地安装。这里以 Windows 11 环境为例介绍如何安装。跟其他任何事情一样，安装 Python 也有多种方式。我这里只介绍最为简单和实用的一种，这就是安装 [Annaconda](https://www.annaconda.com) 公司的一个特殊发行版，它打包了Python（这个本身是开源免费的），以及其他一些工具，让基于Python的编程和数据分析（以及人工智能应用开发）变得从未如此简单。虽然 Python 确实也能做网页，或者一些脚本工具，但目前应用最广泛的领域其实还是数据科学。
> 
> 1. 下载。你可以通过 [官方网站](https://docs.conda.io/en/latest/miniconda.html) 找到下载链接。注意，Annaconda其实提供了两个版本，一个是完整版，一个是精简版。我这里推荐的是精简版。
> 1. 安装。在磁盘上找到刚才下载文件，这是一个以 Annaconda 开头并以 exe 结尾的文件，双击该文件，按照步骤完成安装，在安装向导中全部选择默认选项。
> 1. 验证。安装完成后，点击 Windows 徽标键，然后搜索 `annaconda` 这个关键字，可以找到一个 `Annaconda Prompt` 的工具，以后你就是用这个工具来编写和运行Python程序了。为了方便起见，你可以将它固定在任务栏。
    ![](images/20230127184422.png)  
> 打开这个命令行窗口，输入 `python` 然后回车，你可以看到如下的界面，这就表示安装成功了。
    ![](images/20230127184654.png)  
> 按下 `Ctrl + Z` 组合键可以退出 Python 界面，回到正常的命令行界面。输入 `ipython` 然后回车，可以看到一个不一样的Python窗口。
    ![](images/20230127184847.png)  
> 这个交互式的窗口（正如它的名字那样）给了我们更好的编程和调试体验，我在后续都将以这个模式介绍。

Python 现在有两个主要的分支版本，即 Python2 和 Python3。如果你学过其中的一个，你会发现他们还挺相似的，而且转换起来也不算难。实际上对于初学者来说，他们只有很细微的差别而已。这本书是用 Python3 写的，但也会对 Python2 进行注解。
